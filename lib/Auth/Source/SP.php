...
class SP extends Source
{
	...
	public function __construct($info, $config) {
		...
		$this->metadata = \SimpleSAML\Configuration::loadFromArray($config, 'authsources[' . var_export($this->authId, true) . ']');
	...
	public function getIdPMetadata($entityId) {
		...
		if ($this->idp !== NULL && $this->idp !== $entityId) {
			throw new \SimpleSAML\Error\Exception('Cannot retrieve metadata for IdP ' . var_export($entityId, true) .
		...
		/* Not found. */
		throw new \SimpleSAML\Error\Exception('Could not find the metadata of an IdP with entity ID ' . var_export($entityId, true));
	}
	...
	private function startSSO2(\SimpleSAML\Configuration $idpMetadata, array $state) {
		...
		$ar->setAssertionConsumerServiceURL(\SimpleSAML\Module::getModuleURL('sildisco/sp/saml2-acs.php/' . $this->authId));
	...
	private function startDisco(array $state) {
		...
		$returnTo = \SimpleSAML\Module::getModuleURL('sildisco/sp/discoresp.php', array('AuthID' => $id));
	...
	public function reauthenticate(array &$state) {
		...
		if (sizeof($IDPList) > 1) {
			$state['LoginCompletedHandler'] = array('\SimpleSAML\Module\sildisco\Auth\Source\SP', 'reauthPostLogin');
     ...
     * - 'saml:sp:IdPMetadata': a \SimpleSAML\Configuration object containing the metadata of the IdP that authenticated
    ...
    public static function askForIdPChange(array &$state)
    {
        ...
        if (isset($state['isPassive']) && (bool)$state['isPassive']) {
            ...
            throw new \SimpleSAML\Error\NoPassive('Reauthentication required');
        }
    ...
    public static function reauthLogout(array $state)
    {
        ...
        $state['Responder'] = array('\SimpleSAML\Module\sildisco\Auth\Source\SP', 'reauthPostLogout');
    ...
    public static function reauthPostLogout(\SimpleSAML\IdP $idp, array $state) {
        ...
        $sp = Source::getById($state['saml:sp:AuthId'], '\SimpleSAML\Module\sildisco\Auth\Source\SP');
	...
	public function handleResponse(array $state, $idp, array $attributes) {
		...
		$authProcState = array(
			'saml:sp:IdP' => $idp,
			'saml:sp:State' => $state,
			'ReturnCall' => array('\SimpleSAML\Module\sildisco\Auth\Source\SP', 'onProcessingCompleted'),
	...
	public static function onProcessingCompleted(array $authProcState) {
		...
		$source = Source::getById($sourceId, '\SimpleSAML\Module\sildisco\Auth\Source\SP');
		if ($source === NULL) {
